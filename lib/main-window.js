...
// Dev auto-reload: `npm run dev` runs `vite build --watch`, which rewrites
// compiled/main.js on every save. Watch that file and reload the renderer so
// source edits appear without restarting Electron — the modern replacement for
// the retired webpack-dev-server HMR.
if (process.env.NODE_ENV === 'development') {
  const fs = require('fs')
  const bundlePath = path.resolve(__dirname, '../compiled/main.js')
  let reloadTimer = null
  fs.watchFile(bundlePath, { interval: 250 }, (curr, prev) => {
    if (curr.mtimeMs === prev.mtimeMs) return
    // Debounce so main.js + main.css finish writing before we reload.
    clearTimeout(reloadTimer)
    reloadTimer = setTimeout(() => {
      if (!mainWindow.isDestroyed()) {
        console.log('[dev] renderer bundle changed — reloading window')
