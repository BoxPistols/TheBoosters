...
function httpPost(port, path, body, binaryResponse = false) {
...
async function voicevoxSpeak(text, speakerId, port) {
...
function registerTtsIpc() {
  ...
  ipcMain.handle('tts:speak', async (event, req) => {
    const { text = '', speakerId = DEFAULT_SPEAKER, port = DEFAULT_PORT } =
      req || {}
    if (!text.trim()) return { ok: false, reason: 'empty text' }
    try {
      const wav = await voicevoxSpeak(text, speakerId, port)
